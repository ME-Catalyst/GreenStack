    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-400 mt-1">Time-series database for IoT telemetry</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <Card className="bg-slate-900 border-slate-800">
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-slate-400 text-sm">Writes/sec</p>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
                ...
                {[
                  ...
                  <div key={i} className="flex items-center justify-between p-3 bg-slate-800/50 rounded border border-slate-700">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-sm text-slate-400">{point.time}</span>
                      <Badge variant="outline" className="text-xs">{point.measurement}</Badge>
                    </div>
                    <div className="flex items-center gap-3">
                      <span className="text-white font-mono">{point.value}</span>
                      <span className="text-slate-500 text-sm">{point.device}</span>
                    </div>
                  </div>
                ))}
              </div>
        ...
        <TabsContent value="cli" className="space-y-4">
          <Card className="bg-slate-900 border-slate-800">
            ...
            <CardContent>
              {/* CLI Output */}
              <div className="bg-slate-950 border border-slate-800 rounded-lg p-4 mb-4 h-96 overflow-y-auto font-mono text-sm">
                ...
                <AnimatePresence>
                  {cliHistory.map((entry, i) => (
                    ...
                    >
                      ...
                      {entry.type === 'result' && (
                        <pre className="text-slate-300 ml-4 whitespace-pre-wrap">{entry.content}</pre>
                      )}
                      {entry.type === 'error' && (
              ...
              <div className="flex gap-2">
                <div className="flex-1 relative">
                  ...
                  <Input
                    ...
                    className="pl-8 bg-slate-950 border-slate-800 text-white font-mono"
                  />
                </div>
        ...
        <TabsContent value="query" className="space-y-4">
          <Card className="bg-slate-900 border-slate-800">
            <CardHeader>
              <CardTitle className="text-white">Visual Query Builder</CardTitle>
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select
                    ...
                    className="w-full mt-1 px-3 py-2 bg-slate-950 border border-slate-800 rounded text-white"
                  >
                    {databases.map(db => (
                ...
                <div>
                  <Label className="text-slate-300">Measurement</Label>
                  <select
              ...
              <div className="bg-slate-950 p-4 rounded border border-slate-800">
                <Label className="text-slate-400 text-xs">Generated Query</Label>
        ...
        <TabsContent value="databases" className="space-y-4">
          <div className="grid gap-4">
            {databases.map((db) => (
              <Card key={db.name} className="bg-slate-900 border-slate-800">
                <CardContent className="pt-6">
                  <div className="flex items-center justify-between">
                    <div className="flex-1">
                      ...
                      <div className="grid grid-cols-3 gap-4 mt-4">
                        <div>
                          ...
                          <p className="text-white font-semibold mt-1">{db.measurements}</p>
                        </div>
                        <div>
                          <p className="text-slate-400 text-sm">Series</p>
                          <p className="text-white font-semibold mt-1">{db.series}</p>
                        </div>
                        <div>
                          <p className="text-slate-400 text-sm">Retention</p>
                          <p className="text-white font-semibold mt-1">{db.retention}</p>
                        </div>
                      </div>
                    </div>
                    <div className="flex gap-2">
